...
class LiveViewer:
    def __init__(self, show_camera=True):
        ...
        self.figure = plt.figure("SCARA ACT live viewer", figsize=(12, 6))
        self.robot_ax = self.figure.add_subplot(121)
        self.camera_ax = self.figure.add_subplot(122)
        self.camera_artist = self.camera_ax.imshow(np.zeros((480, 640, 3), dtype=np.uint8))
    ...
    def update(self, image, current_q, predicted_q, chunk_number):
        ...
        if predicted:
            ...
            self.robot_ax.plot(*tools.T, color="tab:red", linewidth=2, label="voorspeld toolpad")
            self.robot_ax.plot(*predicted[-1].T, "o-", color="tab:orange", linewidth=2, label="laatste actie")
        self.robot_ax.plot(*current.T, "o-", color="tab:blue", linewidth=3, label="huidige pose")

        final_j1 = predicted_q[-1][0] if len(predicted_q) else current_q[0]
        self.robot_ax.set(xlim=(-0.75, 0.95), ylim=(-0.85, 0.85),
                          xlabel="X [m]", ylabel="Y [m]",
                          title=f"Chunk {chunk_number}: {len(predicted)} acties | J1 {current_q[0]:.3f} → {final_j1:.3f} m")
